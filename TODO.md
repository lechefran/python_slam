...
- [x] Retain a lightweight timestamped pose/status trajectory for all input frames; keep full descriptors/observations only for keyframes and the bounded tracking cache (opt-in `--tracking-cache-size N`, plus up to 64 recovery representatives). Expiry removes reciprocal observations and unsupported landmarks. Full dashcam coverage regresses to 1,701 poses (95 losses) versus 1,796 (zero losses), so promotion to the default remains blocked. See [bounded feature storage](docs/trajectory-records.md#bounded-feature-storage).
...
- [x] Add live shared-landmark covisibility selection for local BA keyframes/points, retain fixed boundary observations and graph constraints, and report selection evidence. The `--local-map-policy shared` backend is opt-in; temporal selection remains the baseline. Full dashcam qualification finds 16 newly lost frames (1,780 versus 1,796 poses) and larger boundary graphs; resolve these regressions and validate held-out accuracy before promotion. See [local map selection](docs/local-map-selection.md).
- [x] Add opt-in guarded retirement with common-keyframe connectivity evidence, protected boundary/recency observations, pose-preserving reanchoring, stable IDs and reciprocal-link rollback. See [frame retirement](docs/frame-retirement.md). This conservative policy is not a hard memory cap. Synthetic removal is verified; the full dashcam replay retires zero frames under these guards and preserves every baseline pose/outcome.
